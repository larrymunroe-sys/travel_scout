"""CLI runner for the Vintage Clothing Stores Scout Agent."""
...
def main():
    parser = argparse.ArgumentParser(description="Vintage Clothing Stores Scout Agent")
    ...
    try:
        ...
        print("\n==================================================================")
        print("🧥 VINTAGE CLOTHING STORES SCOUT AGENT")
        print("==================================================================")
        print(f"Destination Cities: {', '.join(c['name'] for c in cities)}\n")

        for c in cities:
            print(f"--- Scouting Vintage Fashion in {c['name']}, {c['country']} ---")
            if c["segment_id"] and trip and not args.dry_run:
                res = run_specialist_scout(db, trip.id, c["segment_id"], user_id, "vintage-fashion", max_results=args.max_results)
                print(f"✨ Ingested {res['newly_discovered']} vintage clothing boutiques into Explore & Discover:")
                ...
                raw = hunt_specialist_venues(c["name"], c["country"], "vintage-fashion", max_results=args.max_results)
                enriched = enrich_specialist_venues(raw, c["name"], c["country"])
                print(f"✨ Discovered {len(enriched)} vintage clothing spots:")
                for item in enriched:
                    print(f"   🧥 {item['title']} - {item.get('address') or item.get('neighborhood')}")

        print("\n==================================================================")
        print("✅ Vintage Fashion Scout Complete!")
        print("==================================================================\n")
    finally:
        db.close()
