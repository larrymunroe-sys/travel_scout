"""CLI runner for the Record Stores (Used & New) Scout Agent."""
...
def main():
    parser = argparse.ArgumentParser(description="Record Stores (Used & New) Scout Agent")
    ...
    try:
        ...
        print("\n==================================================================")
        print("📻 RECORD STORES (USED & NEW) SCOUT AGENT")
        print("==================================================================")
        print(f"Destination Cities: {', '.join(c['name'] for c in cities)}\n")

        for c in cities:
            print(f"--- Scouting Vinyl Record Stores in {c['name']}, {c['country']} ---")
            if c["segment_id"] and trip and not args.dry_run:
                res = run_specialist_scout(db, trip.id, c["segment_id"], user_id, "vinyl-records", max_results=args.max_results)
                print(f"✨ Ingested {res['newly_discovered']} record stores into Explore & Discover:")
                for item in res["items"]:
                    print(f"   📻 [FREE / BROWSE] {item['title']}")
                ...
                raw = hunt_specialist_venues(c["name"], c["country"], "vinyl-records", max_results=args.max_results)
                enriched = enrich_specialist_venues(raw, c["name"], c["country"])
                print(f"✨ Discovered {len(enriched)} record stores:")
                for item in enriched:
                    print(f"   📻 {item['title']} - {item.get('address') or item.get('neighborhood')}")

        print("\n==================================================================")
        print("✅ Vinyl Record Scout Complete!")
        print("==================================================================\n")
    finally:
        db.close()
