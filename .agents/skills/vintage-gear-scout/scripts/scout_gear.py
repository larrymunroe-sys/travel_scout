...
def main():
    parser = argparse.ArgumentParser(description="Vintage Music Instruments Scout Agent")
    ...
    try:
        ...
        print("\n==================================================================")
        print("🎸 VINTAGE MUSIC INSTRUMENTS SCOUT AGENT")
        print("==================================================================")
        print(f"Destination Cities: {', '.join(c['name'] for c in cities)}\n")

        for c in cities:
            print(f"--- Scouting Vintage Gear in {c['name']}, {c['country']} ---")
            if c["segment_id"] and trip and not args.dry_run:
                res = run_specialist_scout(db, trip.id, c["segment_id"], user_id, "vintage-gear", max_results=args.max_results)
                print(f"✨ Ingested {res['newly_discovered']} vintage gear shops into Explore & Discover:")
                ...
                raw = hunt_specialist_venues(c["name"], c["country"], "vintage-gear", max_results=args.max_results)
                enriched = enrich_specialist_venues(raw, c["name"], c["country"])
                print(f"✨ Discovered {len(enriched)} vintage gear spots:")
        ...
        print("\n==================================================================")
        print("✅ Vintage Gear Scout Complete!")
        print("==================================================================\n")
    finally:
        db.close()
