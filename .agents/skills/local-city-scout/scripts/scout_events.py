...
from scout.local_agent import (
    ...
    hunt_cultural_events,
    enrich_venue_locations,
    curate_and_ingest_events,
    EVENT_SCAN_QUERIES
)

def main():
    parser = argparse.ArgumentParser(description="Local Destination City Cultural Scout (Subagent Orchestrated)")
    ...
    parser.add_argument("--workers", type=int, default=4, help="Parallel worker threads for subagent pipeline")
    ...
    try:
        ...
        print(f"\n==================================================================")
        print(f"🤖 LOCAL CITY CULTURAL SCOUT AGENT (Subagent Orchestrated)")
        print(f"==================================================================")
        ...
        for city_info in cities_to_scout:
            ...
            # Phase 1: Discover local publications
            ...
            # Phase 2: Execute 3-Subagent Pipeline
            print(f"\n⚡ Spawning Quota-Aware Subagents for {c_name}:")
            print(f"   [1] Cultural Hunter (Flash): Scanning {len(event_types)} categories concurrently...")
            print(f"   [2] Venue Geocoder (Flash-Lite): Enriching addresses and coordinates...")
            print(f"   [3] Itinerary Curator (Flash): Validating, deduplicating, and ingesting...")
            ...
            if c_seg_id and trip and not args.dry_run:
                ...
                for item in result["items"]:
                    ...
                    print(f"      📍 {item.get('address') or item.get('neighborhood')} | Platform: {item.get('source_platform', 'Web')}")
                    if item.get("directions_url"):
                        print(f"      🧭 Transit Route: {item['directions_url']}")
                    if item.get("url"):
                        print(f"      🔗 {item['url']}")
            else:
                # Direct search (dry run or standalone)
                ...
                for item in items:
                    free_tag = "[FREE]" if item.get("is_free") else f"[{item.get('cost', 'Cost varies')}]"
                    print(f"   [{item['category'].upper()}] {free_tag} {item['title']}")
                    print(f"      📍 {item.get('address') or item.get('neighborhood', c_name)} | Source: {item.get('source_platform', 'Web')}")
                    if item.get("directions_url"):
                        print(f"      🧭 Transit Route: {item['directions_url']}")
                    if item.get("url"):
                        print(f"      🔗 {item['url']}")
        ...
        print(f"\n==================================================================")
        print(f"✅ Local Scout Complete! Processed {total_discovered} cultural events.")
        print(f"==================================================================\n")
