"""CLI runner for the Vintage & Modern Home Design Scout Agent."""
...
def main():
    parser = argparse.ArgumentParser(description="Vintage & Modern Home Design Scout Agent")
    ...
    try:
        ...
        print("\n==================================================================")
        print("🛋️ VINTAGE & MODERN HOME DESIGN SCOUT AGENT")
        print("==================================================================")
        print(f"Destination Cities: {', '.join(c['name'] for c in cities)}\n")

        for c in cities:
            print(f"--- Scouting Home Design in {c['name']}, {c['country']} ---")
            if c["segment_id"] and trip and not args.dry_run:
                res = run_specialist_scout(db, trip.id, c["segment_id"], user_id, "home-design", max_results=args.max_results)
                print(f"✨ Ingested {res['newly_discovered']} design showrooms into Explore & Discover:")
                for item in res["items"]:
                    print(f"   🛋️ [{item.get('cost', '$$')}] {item['title']}")
                ...
                raw = hunt_specialist_venues(c["name"], c["country"], "home-design", max_results=args.max_results)
                enriched = enrich_specialist_venues(raw, c["name"], c["country"])
                print(f"✨ Discovered {len(enriched)} home design spots:")
                for item in enriched:
                    print(f"   🛋️ {item['title']} - {item.get('address') or item.get('neighborhood')}")

        print("\n==================================================================")
        print("✅ Home Design Scout Complete!")
        print("==================================================================\n")
    finally:
        db.close()
