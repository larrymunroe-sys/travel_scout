"""CLI runner for the Cooking & Food-Related Stores Scout Agent."""
...
def main():
    parser = argparse.ArgumentParser(description="Cooking & Food-Related Stores Scout Agent")
    ...
    try:
        ...
        print("\n==================================================================")
        print("🔪 COOKING & FOOD-RELATED STORES SCOUT AGENT")
        print("==================================================================")
        print(f"Destination Cities: {', '.join(c['name'] for c in cities)}\n")

        for c in cities:
            print(f"--- Scouting Culinary Goods in {c['name']}, {c['country']} ---")
            if c["segment_id"] and trip and not args.dry_run:
                res = run_specialist_scout(db, trip.id, c["segment_id"], user_id, "culinary-goods", max_results=args.max_results)
                print(f"✨ Ingested {res['newly_discovered']} culinary stores into Explore & Discover:")
                ...
                raw = hunt_specialist_venues(c["name"], c["country"], "culinary-goods", max_results=args.max_results)
                enriched = enrich_specialist_venues(raw, c["name"], c["country"])
                print(f"✨ Discovered {len(enriched)} culinary spots:")
                for item in enriched:
                    print(f"   🔪 {item['title']} - {item.get('address') or item.get('neighborhood')}")

        print("\n==================================================================")
        print("✅ Culinary Goods Scout Complete!")
        print("==================================================================\n")
    finally:
        db.close()
